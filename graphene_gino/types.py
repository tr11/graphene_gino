...
from graphene_gino.special import RelationshipProperty
...
from .utils import get_query, is_mapped_instance
...
def construct_fields(
    ...
    # Gather all the relevant attributes from the Gino model in order
    all_model_attrs = OrderedDict(
        (k, v.column)
        for k, v in model.__dict__.items()
        if isinstance(v, gino.declarative.ColumnAttribute)
    )
    all_model_attrs.update(
        (k, v) for k, v in model.__dict__.items() if isinstance(v, RelationshipProperty)
    )
    ...
    for attr_name, attr in all_model_attrs.items():
        if (only_fields and attr_name not in only_fields) or (
            ...
            continue
        auto_field_names.append(attr_name)
        ...
        if attr_name not in all_model_attrs:
            raise ValueError(
                ("Cannot map ORMField to a model attribute.\n" "Field: '{}.{}'").format(
                    obj_type.__name__, orm_field_name,
                )
            )
    ...
    for orm_field_name in auto_field_names:
    ...
    for orm_field_name, orm_field in orm_fields.items():
        ...
        if isinstance(attr, Column):
            ...
            attr._model = model
            field = convert_relationship(
                attr, registry, connection_field_factory, resolver, **orm_field.kwargs
            )
        else:
...
class GinoObjectTypeOptions(ObjectTypeOptions):
    model = None  # type: gino.Gino.Model
...
class GinoObjectType(ObjectType):
    ...
    def __init_subclass_with_meta__(
        ...
        **options
        ...
        assert isclass(model) and issubclass(model, gino.declarative.Model), (
            "You need to pass a valid Gino Model in " '{}.Meta, received "{}".'
    ...
    async def get_node(cls, info, id):
        try:
            return await cls.get_query(info).get(id)
        except NoResultFound:
            return None

    def resolve_id(self, info):
        inv_map = self._column_name_map
        keys = [
            getattr(self, inv_map.invert_get(x.name))
            for x in self.__table__.primary_key.columns
        ]
