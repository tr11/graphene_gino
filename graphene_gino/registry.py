...
class Registry(object):
    ...
    def register(self, obj_type):
        ...
        if not isinstance(obj_type, type) or not issubclass(obj_type, GinoObjectType):
            raise TypeError(
        ...
        self._registry_str[obj_type._meta.model.__name__] = obj_type

    def get_type_for_model(self, model):
        if isinstance(model, str):
            return self._registry_str.get(model)
